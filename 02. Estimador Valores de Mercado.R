list.of.packages <- c("tidyverse", "corrplot", "lubridat", "fmsb")
new.packages <-
...
Raw$age = as.numeric(lapply(Raw$birth_date, function(X) {
  floor(interval(X, dia) / duration(num = 1, units = "years"))
}))
rm(dia)
...
modeloTop = glm(
  coste ~ age + overall + international_reputation + WORK_DEF + POR + BAL + MEN,
  data = train,
  family = gaussian(link = "identity")
)
summary(modeloTop)
...
modeloTop = glm(
  coste ~ age + overall + international_reputation + POR + BAL + MEN,
  data = train,
  family = gaussian(link = "identity")
)
summary(modeloTop)
...
modeloMP = lm(
  coste ~ age + overall + I(overall ^ 2) + I(overall ^ 3) + international_reputation + POR + BAL + MEN,
  data = train
)
summary(modeloMP)
...
modeloExtra = lm(
  coste ~ special + I(special^2) + I(special^3) + age + overall + I(overall^2) + I(overall^3) + 
...
TOP50 <- df2 %>%
  arrange(desc(ValorMdo)) %>%
  select(name, age, overall, potential, ValorMdo) %>%
  top_n(50)

ggplot(TOP50, aes(
  x = reorder(name, ValorMdo),
  y = ValorMdo,
  fill = overall
)) + geom_bar(stat = "identity") + labs(x = "Top Jugadores", y = "Valor de Mercado", fill = "Puntuación") + coord_flip()
