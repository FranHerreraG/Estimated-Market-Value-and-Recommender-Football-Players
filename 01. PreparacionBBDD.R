list.of.packages <-
  c("tidyverse", "FNN", "shiny", "tibble", "reshape2")
new.packages <-
...
duplicados <-
  as.numeric(names(table(df_aux$ID)[table(df_aux$ID) > 1]))
...
Encontrados <-
  data.frame(
    IDF = c(300, 390, 1166, 1280, 14),
    ID = c(178311, 182882, 189795, 205654, 205498)
  )
...
Raw <- FIFA18v1 %>%
  left_join(df[c("ultimoClub", "nuevoClub", "coste", "mercado", "ID")], by = c("ID" =
                                                                                 "ID"))
